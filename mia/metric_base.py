# This part is modified based on https://github.com/inspire-group/membership-inference-evaluation

import numpy as np

class black_box_benchmarks(object):
    
    def __init__(self, args, shadow_train_performance, shadow_test_performance, 
    ...
    def _mem_inf_via_corr_auc(self):
        ...
        self.plot_auc_curve(self.t_tr_corr,self.t_te_corr,'correctness')
    ...
    def _mem_inf_thre_auc(self, v_name, s_tr_values, s_te_values, t_tr_values, t_te_values):
        ...
        self.plot_auc_curve(t_tr_values,t_te_values,v_name)
        print('For membership inference attack via {n}, the attack acc is {acc:.3f}'.format(n=v_name,acc=mem_inf_acc))
        return mem_inf_acc
    
    def _mem_inf_auc(self, all_methods=True, benchmark_methods=[]):
        if (all_methods) or ('correctness' in benchmark_methods):
            corr_acc = self._mem_inf_via_corr_auc()
        if (all_methods) or ('confidence' in benchmark_methods):
            conf_acc = self._mem_inf_thre_auc('confidence', self.s_tr_conf, self.s_te_conf, self.t_tr_conf, self.t_te_conf)
        if (all_methods) or ('entropy' in benchmark_methods):
            entr_acc = self._mem_inf_thre_auc('entropy', -self.s_tr_entr, -self.s_te_entr, -self.t_tr_entr, -self.t_te_entr)
        if (all_methods) or ('modified entropy' in benchmark_methods):
            modi_acc = self._mem_inf_thre_auc('modified_entropy', -self.s_tr_m_entr, -self.s_te_m_entr, -self.t_tr_m_entr, -self.t_te_m_entr)

        return corr_acc, conf_acc, entr_acc, modi_acc
